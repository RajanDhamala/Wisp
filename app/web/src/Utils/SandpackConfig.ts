const configuredBundlerUrl = import.meta.env.VITE_SANDPACK_BUNDLER_URL;
...
// Plain HTTP LAN origins do not receive Web Crypto in Firefox/Chromium. Sandpack
// still requires SHA-256 only to derive its internal preview identifier.
const ensureLanWebCryptoDigest = () => {
  if (typeof window === "undefined" || window.crypto.subtle) return;

  const digest = async (algorithm: AlgorithmIdentifier, data: BufferSource) => {
    const name = typeof algorithm === "string" ? algorithm : algorithm.name;
    if (name.toUpperCase().replace("_", "-") !== "SHA-256") {
...
export const getSandpackBundlerUrl = () => {
  ensureLanWebCryptoDigest();
  if (configuredBundlerUrl) return configuredBundlerUrl.replace(/\/?$/, "/");
  if (typeof window === "undefined" || window.location.protocol !== "http:")
    return undefined;

  const port = import.meta.env.VITE_SANDPACK_BUNDLER_PORT || "5174";
  return `http://${window.location.hostname}:${port}/`;
};

export const SANDPACK_BUNDLER_TIMEOUT_MS = 60_000;
